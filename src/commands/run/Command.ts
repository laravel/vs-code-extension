...
export default class RunCommand extends Common {
        ...
        this.sub = this.commands$.subscribe(async (commands) => {
            ...
            for (let arg of commandSettings.arguments) {
                let input = await this.getInput(
                    `${arg.name} ${this.getDefaultText(
                        arg,
                    )} ${this.getDescription(arg)}`,
                );
            ...
            for (let opt of commandSettings.options) {
                if (opt.accept_value) {
                    let input = await this.getInput(
                        `${opt.name} ${this.getDefaultText(
                            opt,
                        )} ${this.getDescription(opt)}`,
                    );
                    if (this.isValidInput(input)) {
                        let val = "";
                        opts.push(
                            `${opt.name}${
                                opt.is_value_required
                                    ? `=${
                                          input.length > 0
                                              ? input
                                              : this.toValidInput(opt.default)
                                      }`
                                    : ""
                            }`,
                        );
            ...
            let command = `${commandSettings.name} ${args.join(
                " ",
            )} ${opts.join(" ")}`;
    ...
    private static getDefaultText(obj: any) {
    ...
    private static getDescription(obj: any) {
        return obj.description.toString().length > 0
            ? `(${obj.description})`
            : "";
    }

    private static isValidInput(input: any) {
